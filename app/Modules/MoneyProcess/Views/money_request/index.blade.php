                        ...
                        <table class="table mb-4">
                            ...
                            @if(!empty($rows))
                                @foreach($rows as $element)
                                    <tr>
                                        <td>{{$element->id}}</td>
                                        <td><a href="/users/view/{{$element->user_id}}">{{$element->user->name}}</a></td>
                                        <td> {{$element->available_balance}}</td>
                                        <td> {{$element->requested_amount}}</td>
                                        <td> {!! getRequestStatus($element->status) !!} </td>
                                        <td>
                                            @if($element->status != \App\Modules\BaseApp\Enums\GeneralEnum::TRANSFORMED)
                                                @include('BaseApp::partials.actions' ,['actions'=>['edit'] , $element])
                                            @else
