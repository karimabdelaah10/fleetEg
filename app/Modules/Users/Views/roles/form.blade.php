...
@endphp
@include('BaseApp::form.input',['name'=>'name','type'=>'text','attributes'=>$attributes])

@php
    $attributes['label'] =trans('roles.Display Title');
    $attributes['placeholder'] =trans('roles.Display Title');
@endphp
@include('BaseApp::form.input',['name'=>'display_name','type'=>'text','attributes'=>$attributes])
@if(config('modules'))
    @foreach(config('modules') as $key=>$permissions)
        <h5 class="mg-b-10 mg-t-20 edit-roles-title ">
            <label class="ckbox">
                {!! Form::checkbox('parents',NULL,null,['id'=>$key,'class'=>'parents']) !!}
                <span>{{ucfirst($key)}}</span>
            </label>
        </h5>
        <div class="row edit-roles">
